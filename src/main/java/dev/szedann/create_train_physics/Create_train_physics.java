...
@Mod(Create_train_physics.MODID)
public class Create_train_physics {
    ...
    // Create a Deferred Register to hold CreativeModeTabs which will all be registered under the "create_train_physics" namespace
    ...
    // Creates a new food item with the id "create_train_physics:example_id", nutrition 1 and saturation 2
//    public static final DeferredItem<Item> EXAMPLE_ITEM = ITEMS.registerSimpleItem("example_item", new Item.Properties().food(new FoodProperties.Builder().alwaysEdible().nutrition(1).saturationModifier(2f).build()));

    // Creates a creative tab with the id "create_train_physics:example_tab" for the example item, that is placed after the combat tab
    public static final DeferredHolder<CreativeModeTab, CreativeModeTab> EXAMPLE_TAB = CREATIVE_MODE_TABS.register("example_tab", () -> CreativeModeTab.builder().title(Component.translatable("itemGroup.create_train_physics")).withTabsBefore(CreativeModeTabs.COMBAT).icon(() -> TRAIN_ENGINE_BLOCK_ITEM.get().getDefaultInstance()).displayItems((parameters, output) -> {
        output.accept(TRAIN_ENGINE_BLOCK_ITEM.get()); // Add the example item to the tab. For your own tabs, this method is preferred over the event
    }).build());

    // The constructor for the mod class is the first code that is run when your mod is loaded.
    // FML will recognize some parameter types like IEventBus or ModContainer and pass them in automatically.
    public Create_train_physics(IEventBus modEventBus, ModContainer modContainer) {
...
//        modEventBus.addListener(this::commonSetup);
        ...
        // Note that this is necessary if and only if we want *this* class (Create_train_physics) to respond directly to events.
